...
def web_catalog(it_doc):
    ...
    if it_doc.show_in_website == 1:
        ...
        FROM `tabItem Reorder` WHERE parent ='%s' """ % (it_doc.name), as_list=1)
...
def validate_restriction(it_doc):
    if it_doc.has_variants == 1:
        ...
        for d in it_doc.item_variant_restrictions:
            if d.is_numeric == 1:
                if d.allowed_values:
                    frappe.throw("Allowed Values field not allowed for numeric attribute {0}".format(d.attribute))
            elif d.is_numeric == 0:
                if d.rule:
                    frappe.throw("Rule not allowed for non-numeric attribute {0}".format(d.attribute))
    ...
    if it_doc.item_defaults:
        ...
        for d in it_doc.item_defaults:
            if d.default_warehouse:
                ...
                frappe.throw("Default Warehouse is Mandatory for Item Code: {}".format(it_doc.name))
            if d.default_price_list:
                def_price_list = d.default_price_list
            else:
                if it_doc.is_sales_item == 1:
                    frappe.throw("Default Price List is Mandatory for Item Code: {}".format(it_doc.name))
...
def make_route(it_doc):
    ...
    if it_doc.has_variants == 1:
        it_doc.route = frappe.db.get_value('Item Group', it_doc.item_group, 'route') + '/' + it_doc.scrub(route_name)
    else:
        if it_doc.variant_of:
            it_doc.route = frappe.db.get_value('Item', it_doc.variant_of, 'route') + '/' + it_doc.scrub(route_name)
...
def validate_variants(it_doc, comm_type=None):
    ...
    if it_doc.show_in_website == 1:
        if it_doc.image is None:
            frappe.throw("For Website Items, Website Image is Mandatory for Item Code {}".format(it_doc.name))
    if it_doc.variant_of:
        ...
        for i in range(len(template_attribute)):
            if len(template_attribute) == len(variant_attribute) and \
                ...
                frappe.throw("Item Code: {0} number of attributes not as per the template".format(it_doc.name))
        ...
        for d in it_doc.attributes:
            chk_numeric = frappe.db.get_value("Item Attribute", d.attribute, "numeric_values")
            ...
            if chk_numeric == 1:
                ...
                if template_restricted_attributes.get(d.attribute, {}).get('allows', False):
                    if d.attribute_value not in template_restricted_attributes.get(d.attribute, {}) \
                                    .get('allows', []):
                        frappe.throw("Item Code: {0} Attribute value {1} not allowed".format(it_doc.name,
                                                                                                                                                                 d.attribute_value))

        # Check the limit in the Template
        limit = template.variant_limit
        actual = frappe.db.sql("""SELECT count(name) FROM `tabItem` WHERE variant_of = '%s'""" % template.name,
                                                   as_list=1)
        ...
        if check:
            if actual[0][0] > limit:
                frappe.throw(("Template Limit reached. Set Limit = {0} whereas total number of variants = {1} "
                                          "increase the limit to save the variant").format(limit, actual[0][0]))
            else:
                pass
        else:
            if actual[0][0] >= limit:
                frappe.throw(("Template Limit reached. Set Limit = {0} whereas total number of variants = {1} "
                                          "increase the limit to save New Item Code").format(limit, actual[0][0]))
...
def check_item_defaults(template, variant, comm_type=None):
    field_list = ["company", "default_warehouse", "default_price_list", "income_account", "buying_cost_center", "selling_cost_center"]
...
def copy_item_defaults(template, variant, field_list, comm_type=None):
    ...
    if comm_type == "backend":
        it_def_name = frappe.db.sql(
                """SELECT name FROM `tabItem Default` WHERE parent= '%s' AND parenttype='Item'""" % (variant.name),
                as_list=1)
        if it_def_name:
            for t in template.item_defaults:
                for f in field_list:
