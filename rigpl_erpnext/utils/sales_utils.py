	...
	for items in document.items:
		if items.get(so_field):
			...
			if document.customer_address != so_doc.customer_address:
				frappe.throw("Customer Address mentioned in {} {} does not match \
					with Customer Address of SO# {} mentioned in Row# {}".\
					format(document.doctype, document.name, items.get(so_field), \
						items.idx))
			if document.shipping_address_name != so_doc.shipping_address_name:
				frappe.throw("Shipping Address mentioned in {} {} does not match \
					with Shipping Address of SO# {} mentioned in Row# {}".\
					format(document.doctype, document.name, items.get(so_field), \
						items.idx))
			...
			if it.get(so_field):
				...
				if it.idx > 1:
					if it.get(so_field) not in so_list:
						frappe.throw("Customer {} Follows Strict PO Rules hence \
							Row {} is Rejected. Make new {} for SO# {}".\
							format(document.customer, it.idx, document.doctype, it.get(so_field)))
