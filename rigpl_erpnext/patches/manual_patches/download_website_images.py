...
def execute():
    ...
    attached_to_single_dt = ["Website Slideshow", "Website Settings", "Web Page", "Blog Post", "About Us Settings"]
    multi_dt = ["Item Group", "Item"]

    for dt in attached_to_single_dt:
        ss_files = frappe.db.sql("""SELECT file_url, file_name FROM `tabFile` WHERE attached_to_doctype = '%s' AND is_private = 0""" % dt, as_dict=1)
        for ff in ss_files:
    ...
    for dt in multi_dt:
        if dt == "Item Group":
            ss_files = frappe.db.sql("""SELECT fd.file_url, fd.file_name FROM `tabFile` fd, `tab%s` it 
                WHERE fd.attached_to_doctype = '%s' AND fd.attached_to_name = it.name AND it.show_in_website = 1
                AND fd.is_private = 0""" % (dt, dt), as_dict=1)
        else:
            ss_files = frappe.db.sql("""SELECT fd.file_url, fd.file_name FROM `tabFile` fd, `tab%s` it 
                WHERE fd.attached_to_doctype = '%s' AND fd.attached_to_name = it.name AND (it.show_in_website = 1 OR it.show_variant_in_website = 1)
                AND fd.is_private = 0 """ % (dt, dt), as_dict=1)
        for ff in ss_files:
            web_add = frm_web + quote(ff.file_url)
            local_path = base_path + '/' + ff.file_name
