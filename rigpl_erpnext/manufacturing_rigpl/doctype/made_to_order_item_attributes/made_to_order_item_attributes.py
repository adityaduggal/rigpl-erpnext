...
class MadetoOrderItemAttributes(Document):
    def validate(self):
        ...
        if self.reference_spl:
            self.reference_spl = ""
        other_specials = frappe.db.sql(
            """SELECT name FROM `tabMade to Order Item Attributes` WHERE sales_order =
            ...
            frappe.throw(
                "{} Already Exists".format(
                    frappe.get_desk_link(
                        "Made to Order Item Attributes", other_specials[0].name
                    )
                )
            )
    ...
    def copy_attributes_from_another_spl(self):
        if self.reference_spl:
            if self.reference_spl == self.name:
                frappe.throw(
                    "Cannot Select Name of Special Item Attribute as {}".format(
                        self.name
                    )
                )
            else:
