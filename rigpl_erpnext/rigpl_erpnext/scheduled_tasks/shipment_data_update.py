...
def get_all_ship_data():
    ...
    pending_ships = frappe.db.sql("""SELECT ctrack.name FROM `tabCarrier Tracking` ctrack, `tabTransporters` tpt 
        WHERE ctrack.posted_to_shipway = 1 AND ctrack.docstatus != 2 AND tpt.fedex_credentials = 0 
        AND tpt.name = ctrack.carrier_name
        ORDER BY ctrack.creation DESC """, as_list=1)
...
def pushOrderData(track_doc):
    ...
    trans_doc = frappe.get_doc('Transporters', track_doc.carrier_name)
    if track_doc.get("__islocal") != 1 and track_doc.posted_to_shipway == 0 and trans_doc.fedex_credentials != 1:
...
def getOrderShipmentDetails(track_doc):
    ...
    fedex_cred = frappe.get_value("Transporters", track_doc.carrier_name, "fedex_credentials")
