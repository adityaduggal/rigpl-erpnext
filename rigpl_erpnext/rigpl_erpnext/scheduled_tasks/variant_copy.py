...
def check_wrong_variants():
	limit = frappe.db.get_single_value("Stock Settings", "automatic_sync_field_limit")
	...
	if is_sync_allowed == 1:
		...
		for t in templates:
			...
			if fields_edited <= limit:
				...
				print ("Limit of " + str(limit) + " fields reached. Run again for more updating")
				exit()
