...
from rigpl_erpnext.rigpl_erpnext.scheduled_tasks.shipment_data_update import getOrderShipmentDetails, pushOrderData
...
class CarrierTracking(WebsiteGenerator):
    ...
    def get_status(self):
        if self.status is not None and self.status != "Not Booked":
    ...
    def validate(self):
        ...
        if trans_doc.fedex_credentials == 1:
            ...
            self.courier_charges_validation(trans_doc)
    ...
    def update_fields(self, trans_doc):
        if self.document == 'Sales Invoice':
            ...
            if not self.contact_person:
                def_contact = frappe.db.sql("""SELECT con.nameFROM `tabContact` con, `tabDynamic Link` dl 
    ...
    def courier_charges_validation(self, trans_doc):
        if trans_doc.max_percent_of_invoice_value is not None and self.shipment_cost is not None:
            if self.amount > 0:
                if self.shipment_cost / self.amount * 100 > trans_doc.max_percent_of_invoice_value:
                    if self.purpose != "SOLD" and self.document != "Sales Invoice":
                        frappe.msgprint("Permissible Courier Cost is High. Make Sure you Charge Courier for "
                                        "Minimum Amount of {}".format(self.shipment_cost), title="Warning From Admin")
                    elif self.purpose == "SOLD" and self.document == "Sales Invoice":
                        if self.courier_charged < self.shipment_cost and self.bypass_courier_charged_check != 1:
                            frappe.throw("Not Allowed. Courier Charged in Invoice {} is Lower than Cost "
                                         "of ₹ {}".format(self.document_name, self.shipment_cost), "Fatal Error")
        ...
        if self.receiver_document == "Customer":
            ...
            for sp in cust_doc.sales_team:
                ...
                if emp_doc.status != 'Left':
                    ...
                    frappe.throw(("Employee {} Linked with Sales Person {} for Customer {} in Carrier Tracking {} "
                                  "is Not Active".format(employee, sp.sales_person, self.receiver_name, self.name)))
