...
class BRCMEISTracking(Document):
    ...
    def meis_validate(self):
        ...
        if self.meis_authorization_no:
            ...
            pno = re.compile("[0-9]{10}")
            if not pno.match(str(self.meis_authorization_no)):
        ...
        if self.export_or_import == 'Export':
            ...
            if self.shipping_bill_date:
                ...
                if diff.days > 20 or diff.days < -20:
                    ...
                    message = f"Max Difference Allowed is 20 days between Shipping Bill Date \
                    {self.get_formatted('shipping_bill_date')} and Reference Date \
                    {self.get_formatted('reference_date')}"
                    if "System Manager" in user_role or frappe.session.user == "Administrator":
    ...
    def import_export_docs_query(reference_document):
        """
        Query for Sales or Purchase Invoice IMPORT or EXPORT
        """
