...
def validate(doc, method):
    if doc.is_subcontracting_warehouse == 1:
        ...
        other_subcontracting_warehouses = frappe.db.sql("""SELECT name FROM `tabWarehouse` WHERE docstatus = 0 
        AND is_subcontracting_warehouse = 1 AND name != '%s'""" % doc.name, as_list=1)
    ...
    if doc.is_group == 0:
        ...
        if len(other_serial) > 0:
            frappe.throw(
                "Listing Serial: {} already Assigned to Warehouse {}".format(doc.listing_serial, other_serial[0][0]))
        ...
        if doc.short_code:
            ...
            if len(oth_short_code) > 0:
                frappe.throw(
                    "Short Code: {} already Assigned to Warehouse {}".format(doc.short_code, oth_short_code[0][0]))
        else:
