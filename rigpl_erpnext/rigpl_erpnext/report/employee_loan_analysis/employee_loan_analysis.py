...
def get_opening(filters):
	...
	for d in frappe.db.sql("""SELECT eld.employee, SUM(eld.loan_amount) as op_loan
		FROM `tabEmployee Advance RIGPL` el, `tabEmployee Loan Detail` eld
...
def get_details(filters):
	...
	loan = frappe.db.sql("""SELECT el.posting_date, eld.loan_amount, 0, 'Employee Advance RIGPL', el.name
		FROM `tabEmployee Advance RIGPL` el, `tabEmployee Loan Detail` eld
