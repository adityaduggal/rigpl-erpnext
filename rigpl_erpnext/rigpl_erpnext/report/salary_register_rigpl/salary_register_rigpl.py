...
def get_columns(salary_slips, filters):
    """
    Returns columns based on Filters
    """
    if filters.get("without_salary_slip") == 1:
        columns = [
                _("Employee") + ":Link/Employee:100", _("Employee Name") + "::140",
...
def get_employee(filters, conditions_emp):
    """
    Returns Employees based on Filters
    """
...
def get_salary_slips(filters, conditions_ss, emp_lst):
    """
    Returns Salary Slips based on Filters and Employee List
    """

    salary_slips = frappe.db.sql("""SELECT *
        FROM `tabSalary Slip` ss
        WHERE ss.docstatus < 3  {condition} AND ss.employee IN (%s)
...
def get_ss_earning_map(salary_slips, filters):
    """
    Gets Salary Slip Earning Maps for Salary Slips
    """
    if filters.get("bank_only") == 1:
...
def get_ss_ded_map(salary_slips):
    """
    Gets Deduction Maps for Given Salary Slips
    """
...
def get_ss_cont_map(salary_slips):
    """
    Gets Contribution Maps for Given Salary Slips
    """
...
def get_conditions(filters):
    """
    Returns conditions based on Filters for both Salary Slips and Employees
    """
    conditions_ss = ""
    conditions_emp = ""
    ...
    if filters.get("docstatus") == "Submitted":
        conditions_ss += " AND ss.docstatus = 1"
    elif filters.get("docstatus") == "Draft":
        conditions_ss += " AND ss.docstatus = 0"
    elif filters.get("docstatus") == "Cancelled":
        conditions_ss += " AND ss.docstatus = 2"
