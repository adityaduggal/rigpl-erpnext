...
def execute(filters=None):
	...
	ot_map = get_attendance_list(conditions,filters)[1]
	...
	for emp in sorted(ot_map):
		...
		for day in range(filters["total_days_in_month"]):
			status = ot_map.get(emp).get(day + 1, "None")
...
def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(att_date) as day_of_month,
		status, overtime from tabAttendance where docstatus = 1 %s order by employee, att_date""" %
	...
	frappe.msgprint(ot_map)

	return att_map, ot_map
