...
def get_entries(filters, conditions):
    data = []
    if filters.get("type") == "Only Addresses":
        query = """SELECT cu.name,  cu.customer_name, cu.customer_type, cu.customer_group, cu.territory, ad.name, 
        ad.address_type, ad.address_line1, ad.address_line2, ad.city, ad.state, ad.pincode, ad.country, 
        ad.email_id, ad.phone, ad.fax, ad.gstin, ad.excise_no, ad.service_tax_no FROM `tabCustomer` cu, 
        `tabAddress` ad, `tabDynamic Link` dl WHERE dl.link_doctype = 'Customer' AND dl.link_name = cu.name 
        AND dl.parenttype = 'Address' AND dl.parent = ad.name %s""" % conditions
        data = frappe.db.sql(query, as_list=1)

    elif filters.get("type") == "Only Contacts":
        query = """SELECT cu.name,  cu.customer_name, cu.customer_type, cu.customer_group, cu.territory, con.name, 
        con.first_name, con.last_name, con.email_id, con.phone, con.mobile_no, con.department, con.designation, 
        con.notes, con.user, con.birthday, con.anniversary FROM `tabCustomer` cu, `tabContact` con, 
        `tabDynamic Link` dl WHERE dl.link_doctype = 'Customer' AND dl.link_name = cu.name 
        AND dl.parenttype = 'Contact' AND dl.parent = con.name %s""" % conditions
        data = frappe.db.sql(query, as_list=1)
    return data
