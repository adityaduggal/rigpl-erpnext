...
def get_sl_entries(filters):
	...
	if (filters.get("from_date")):
		...
		webnotes.msgprint ("Please select from date first", raise_exception=1)

	DN_Qty= webnotes.conn.sql("""select sle.item_code, sum(sle.actual_qty)*-1
	...
	cust_nos = webnotes.conn.sql("""select sod.item_code, count(DISTINCT(so.customer))
	...
	data = webnotes.conn.sql("""select it.name, it.description, if(it.re_order_level=0,NULL,it.re_order_level), 
	...
	SE_Qty = webnotes.conn.sql("""select sted.item_code, sum(sted.qty)
	...
	No_of_DN = webnotes.conn.sql("""select sle.item_code, (count(DISTINCT sle.voucher_no))*1
	from `tabStock Ledger Entry` sle where sle.voucher_type = "Delivery Note" and sle.is_cancelled = "No" %s
	group by sle.item_code order by sle.item_code""" 
	% conditions, as_list=1)
	...
	#webnotes.msgprint(type(data[1][3]))
